...
seatyears <- tapply(senate$sen_RaceDate, INDEX=list(senate$statenm), FUN=seats)
...
countyplus[, names(countyplus) %in% c("x", "decade", "year")]
...
for(i in 1:nrow(countyplus)) {
  ...
  if(is.na(countyplus$gov_RepVotesMajorPercent[i])) {
  ...
  if(is.na(countyplus$pres_RepVotesTotalPercent[i])) {

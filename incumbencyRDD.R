...
rd2$coef <- rd2$coef / (leftintercept + rightintercept)
rd2$ci <- rd2$ci / (leftintercept + rightintercept)

out <- list(`Full Sample` = rd2)
stage1 <- matrix(ncol=5)
stats <- round(c((leftintercept + rightintercept), rd2$beta_p_l[1], rd2$N_h[1], rd2$N_h[2], rd2$bws[1,1]), digits=3)
...
for(j in 1:length(unique(elections$decade))) {
  ...
  rd1 <- rdrobust(y = electemp$incumb_running_tplus1, x=electemp$demmv) # first model - outcome is incumbent running in t+1
  leftintercept <- rd1$beta_p_l[1]
  rightintercept <- rd1$beta_p_r[1]
  
  rd2 <- rdrobust(y=electemp$con_demshare_tplus1, x=electemp$demmv) # second model - outcome is Dem vote share in t+1
  rd2$coef <- rd2$coef / (leftintercept + rightintercept)
  rd2$ci <- rd2$ci / (leftintercept + rightintercept)
  ...
  if(j==length(unique(elections$decade))) {
    modelsummary(out, output = "../results/RDestimates_decade_voteshare.tex", coef_omit= "Intercept|factor|Conventional|Bias",
                 stars=F, gof_omit = "Std|BIC|AIC|Log|Adj|R2|F|n", statistic = "conf.int",
                 add_rows = data.frame(cbind(c("Incumb. RD Estimate", "Intercept left of CO", "Eff. N left of cutoff", "Eff. N right of cutoff", "Bandwidth"), 
...
rd2 <- rdrobust(y=elections$demwin_tplus1, x=elections$demmv) # second model - outcome is Dem vote share in t+1
rd2$coef <- rd2$coef / (leftintercept + rightintercept)
rd2$ci <- rd2$ci / (leftintercept + rightintercept)
...
for(j in 1:length(unique(elections$decade))) {
  ...
  rd2 <- rdrobust(y=electemp$demwin_tplus1, x=electemp$demmv) # second model - outcome is Dem vote share in t+1
  rd2$coef <- rd2$coef / (leftintercept + rightintercept)
  rd2$ci <- rd2$ci / (leftintercept + rightintercept)
  ...
  if(j==length(unique(elections$decade))) {
    modelsummary(out, output = "../results/RDestimates_decade_windummy.tex", coef_omit= "Intercept|factor|Conventional|Bias", 
                 stars=F, gof_omit = "Std|BIC|AIC|Log|Adj|R2|F|n", statistic = "conf.int",
                 add_rows = data.frame(cbind(c("Incumb. RD Estimate", "Intercept left of CO", "Eff. N left of cutoff", "Eff. N right of cutoff", "Bandwidth"), 
                                             matrix(stage1[-1,], nrow = ncol(stage1), byrow = T))))
  }
...
for(j in 1:length(bins)) {
  ...
  if(j==length(bins)) {
    modelsummary(out, output = "../results/RDestimates_incumblgth_voteshare.tex", coef_omit= "Intercept|factor|Conventional|Bias", 
                 stars=F, gof_omit = "Std|BIC|AIC|Log|Adj|R2|F|n", statistic = "conf.int",
                 add_rows = data.frame(cbind(c("Incumb. RD Estimate", "Intercept left of CO", "Eff. N left of cutoff", "Eff. N right of cutoff", "Bandwidth"), 
                                             matrix(stage1[-1,], nrow = ncol(stage1), byrow = T))))
  }
...
for(j in 1:length(bins)) {
  ...
  if(j==length(bins)) {
    modelsummary(out, output = "../results/RDestimates_incumblgth_windummy.tex", coef_omit= "Intercept|factor|Conventional|Bias", 
                 stars=F, gof_omit = "Std|BIC|AIC|Log|Adj|R2|F|n", statistic = "conf.int",
                 add_rows = data.frame(cbind(c("Incumb. RD Estimate", "Intercept left of CO", "Eff. N left of cutoff", "Eff. N right of cutoff", "Bandwidth"), 
                                             matrix(stage1[-1,], nrow = ncol(stage1), byrow = T))))
  }
